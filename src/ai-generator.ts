...
export class AiParagraphGenerator {
  ...
  async generateNaturalParagraph(vocabulary: VocabularyItem[], wordCount: number = 12): Promise<string> {
    const selectedWords = this.selectBalancedWords(vocabulary, wordCount);
    console.log(`Selected ${selectedWords.length} words for AI generation:`, selectedWords.map(w => w.characters).join(', '));
    const vocabularyList = selectedWords.map(word => 
      `${word.characters} (${word.readings[0]} - ${word.meanings.slice(0, 2).join(', ')})`
    ).join('\n');

    const prompt = `Create a natural, grammatically correct Japanese paragraph using the vocabulary words provided below. Requirements:

1. Use natural Japanese sentence structure (Subject-Object-Verb)
2. Try to use most or all of the provided vocabulary words
3. Write 2-3 sentences that form a coherent story or scene
...
Please write a natural Japanese paragraph that tells a simple story using these words. Respond with ONLY the Japanese text.`;
  ...
  private selectBalancedWords(vocabulary: VocabularyItem[], count: number): VocabularyItem[] {
    ...
    const others = vocabulary.filter(w => !this.isVerb(w) && !this.isNoun(w) && !this.isAdjective(w));
  ...
  private isAdjective(word: VocabularyItem): boolean {
    return word.characters.endsWith('い') && word.characters.length > 1 &&
           !word.characters.endsWith('ない') && !this.isVerb(word);
  }

  createFullOutput(japanese: string, english: string, selectedWords: VocabularyItem[]): string {
